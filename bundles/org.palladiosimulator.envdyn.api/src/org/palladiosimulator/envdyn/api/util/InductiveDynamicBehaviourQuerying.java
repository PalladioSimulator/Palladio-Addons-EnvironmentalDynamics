...
public class InductiveDynamicBehaviourQuerying {
    ...
    public Set<InterTimeSliceInduction> getInterTimeSliceInductions() {
        return dynamics.getTimeSliceInductions()
            .stream()
            .filter(InterTimeSliceInduction.class::isInstance)
            .map(InterTimeSliceInduction.class::cast)
    ...
    public Set<IntraTimeSliceInduction> getIntraTimeSliceInductions() {
        return dynamics.getTimeSliceInductions()
            .stream()
            .filter(IntraTimeSliceInduction.class::isInstance)
            .map(IntraTimeSliceInduction.class::cast)
            .collect(Collectors.toCollection(LinkedHashSet::new));
    }

    public Set<TemplateVariable> filterInterfaceVariables() {
        return getTemporalRelations().stream()
    ...
    public static TemplateVariable getTarget(TemporalRelation relation) {
        if (PersistenceRelation.class.isInstance(relation)) {
            return PersistenceRelation.class.cast(relation)
                .getInterfaceVariable();
        } else {
            return TimeSliceRelation.class.cast(relation)
                .getTarget();
        }
    }
    ...
    private Set<TemporalRelation> getTemporalRelations() {
        return dynamics.getTimeSliceInductions()
            ...
            .filter(InterTimeSliceInduction.class::isInstance)
            .map(InterTimeSliceInduction.class::cast)
            .flatMap(each -> each.getTemporalStructure()
                .stream())
            .collect(Collectors.toCollection(LinkedHashSet::new));
    }
