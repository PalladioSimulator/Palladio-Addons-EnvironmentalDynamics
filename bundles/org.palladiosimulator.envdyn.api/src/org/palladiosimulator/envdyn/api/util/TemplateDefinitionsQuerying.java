	...
	private class QueryingScope {
		...
		private void adjustRelationScope(TemplateVariable base, TemplateVariable sub) {
			for (Relation each : Sets.newHashSet(relationScope)) {
				...
				} else if (each instanceof PresistenceRelation) {
					adjustPersistenceRelation(base, sub, (PresistenceRelation) each);
				}
			}
		}

		private void adjustPersistenceRelation(TemplateVariable base, TemplateVariable sub,
				PresistenceRelation relation) {
		...
		private Relation copyAndAdjust(PresistenceRelation relation, TemplateVariable sub) {
			PresistenceRelation adjustedCopy = (PresistenceRelation) EcoreUtil.copy(relation);
